...
contract TaskAssigner {
    ...
    function updateTaskStatus(uint taskID, uint stage) external {
        ...
        require(robotID != -1, 'No such robot!');

        //check task assigner
        require(tasks[taskID].assigner == uint(robotID), 'This robot is not doing this task!');
    ...
    function getOwnTask() public returns (Task memory) {
